...
"Must be a number"),a=!0)});checkTerms()||(a=!0);a?($.alert({title:"Error",type:"red",content:"Please correct the error(s) in the form to continue placing an order"}),disableInput(!1)):(submitted=!0,prepareForm(),$(".real-form").submit())},checkTerms=function(){return $("#terms").is(":checked")?!0:($("#terms").parent().append('<div class="error-msg"><small class="text-danger">You need to agree with the terms below to proceed and make a Donation.</small></div>'),!1)},uuid=function(){return Math.floor(Math.random()*
Date.now())},prepareForm=function(){$(".input-form").each(function(){$(".real-form").find("#"+$(this).attr("id").replace("input-","")).hasClass("populatable")&&$(".real-form").find("#"+$(this).attr("id").replace("input-","")).val($(this).val())});prepareBeneficiaries();var a=uuid(),b=0,c=[];$(".beneficiaries .beneficiary").each(function(){b+=parseInt($(this).find(".input-qty").val());c.push(4200*parseInt($(this).find(".input-qty").val()))});$("#amount").val(c.join(" / "));$(".real-form #total-box").val(b);
setCookie("reference",a,30);setCookie("total",getOrderTotal(),30);$(".real-form #reference").val(a)},prepareBeneficiaries=function(){var a=[],b=[],c=[],d=[],e=[],f=[],g=[];$(".beneficiaries .beneficiary").each(function(){a.push($(this).find(".input-beneficiary").val());b.push($(this).find(".input-qty").val());c.push($(this).find(".input-point-person").val());d.push($(this).find(".input-point-contact").val());e.push($(this).find(".input-shipping-mode").val());"pickup"==$(this).find(".input-shipping-mode").val()?
(g.push($(this).find(".input-pickup:checked").val()),f.push("-NONE-")):"delivery"==$(this).find(".input-shipping-mode").val()&&(f.push($(this).find(".input-address").val()),g.push("-NONE-"))});$(".real-form #beneficiary").val(a.join(" / "));$(".real-form #qty").val(b.join(" / "));$(".real-form #point-person").val(c.join(" / "));$(".real-form #point-contact").val(d.join(" / "));$(".real-form #shipping-mode").val(e.join(" / "));$(".real-form #address").val(f.join(" / "));$(".real-form #pickup-point").val(g.join(" / "))},
